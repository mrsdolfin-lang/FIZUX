...
function callWithModel(modelId, msgs, taskModel) {
  // Manual key takes priority over system keys
  if(manualTextKey && manualTextKey.length > 10){
    return callManualAPI(msgs, manualTextKey, manualProvider, manualModel || 'gpt-4o-mini');
  }
...
function FIZUXApp(){
  ...
  useEffect(function(){
    ...
    // Load manual custom API keys if set by user
    try{
      var manText = localStorage.getItem('fizux:manual_text_key');
      var manImg = localStorage.getItem('fizux:manual_img_key');
      var manModel = localStorage.getItem('fizux:manual_model');
      var manProvider = localStorage.getItem('fizux:manual_provider');
      if(manText) setManualTextKey(manText);
      if(manImg) setManualImgKey(manImg);
      if(manModel) setManualModel(manModel);
      if(manProvider) setManualProvider(manProvider);
    }catch(e){}
  },[]);
...
if(screen==='login'){
    ...
    useEffect(function(){
      function onReady(){
        ...
        if(loginType === 'free'){
          ...
          // Ukey: show ukey input panel (screen stays login but card visible)
          setScreen('ukey-entry');
        }
      }
    ...
    return h('div',{style:{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',padding:'20px',background:'transparent',position:'relative',overflow:'hidden'},className:'screen-fade'},
      ...
      h('div',{style:{width:'100%',maxWidth:'400px'}},
        ...
        h('div',{style:{textAlign:'center',marginBottom:'28px'}},
          h('div',{className:'logo-3d float-3d',style:{width:'80px',height:'80px',borderRadius:'24px',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'36px',fontWeight:900,color:'#FFFFFF',margin:'0 auto 18px'}},'F'),
        ...
        // Card
        h('div',{className:'login-card-3d',style:{borderRadius:'24px',padding:'32px',animation:'fadeUp 0.5s cubic-bezier(0.16,1,0.3,1) 0.1s both',transform:'perspective(1000px) translateZ(0)'}},
          // Google btn
          h('button',{
  ...
  function renderMsg(msg,i){
    ...
    if(!isUser && msg.content === '__IMAGES__' && msg.images){
      return h('div',{key:i,className:'msg-appear',style:{padding:'4px 16px',maxWidth:'720px',margin:'0 auto'}},
        h('div',{className:'msg-ai-3d',style:{
  ...
  return h('div',{
    ...
    // Toast
    toast.msg && h('div',{className:'toast-3d',style:{position:'fixed',top:'16px',left:'50%',transform:'translateX(-50%)',border:'1.5px solid '+(toast.type==='err'?RED:GREEN),borderRadius:'12px',padding:'10px 20px',fontSize:'13px',fontWeight:500,color:toast.type==='err'?RED:GREEN,zIndex:9999,whiteSpace:'nowrap'}},toast.msg),
    OnboardModal,
    SettingsModal,
    LimitModal,
    CompareModal,
    ...
    // Sidebar
    h('div',{className:'sidebar-3d',style:{position:'fixed',left:0,top:0,bottom:0,width:sidebar?'275px':'0',transition:'width 0.25s ease',overflow:'hidden',zIndex:99}},
      h('div',{style:{width:'275px',height:'100%',display:'flex',flexDirection:'column',overflow:'hidden'}},
        ...
        h('div',{style:{padding:'14px 12px 8px',display:'flex',alignItems:'center',gap:'8px'}},
          h('div',{className:'logo-3d',style:{width:'30px',height:'30px',borderRadius:'10px',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:800,fontSize:'14px',color:'#FFFFFF'}},'F'),
        ...
        h('div',{style:{flex:1,overflowY:'auto',padding:'0 10px'}},
          ...
          FEATURES.map(function(f){
            return h('button',{key:f.id,onClick:function(){setSelectedFeat(f);setSidebar(false);if(inputRef.current)inputRef.current.focus();},
              className:'feat-row-3d sidebar-item',style:{width:'100%',padding:'7px 8px',borderRadius:'8px',border:'none',color:TEXT,cursor:'pointer',textAlign:'left',display:'flex',alignItems:'center',gap:'7px',marginBottom:'1px',fontFamily:"Inter,system-ui,sans-serif",transition:'all 0.15s ease'}},
    ...
    // Header
    h('div',{className:'header-3d',style:{padding:'0 12px',height:'64px',display:'flex',alignItems:'center',gap:'8px',flexShrink:0}},
      ...
      h('div',{className:'mode-toggle-3d',style:{display:'flex',gap:'3px',borderRadius:'10px',padding:'3px'}},
    ...
    // Messages area
    h('div',{ref:chatAreaRef,className:'chat-scroll',style:{flex:1,overflowY:'auto',padding:'0'}},
      h('div',{style:{maxWidth:'720px',margin:'0 auto'}},
        ...
        loading && h('div',{style:{padding:'4px 16px',display:'flex',justifyContent:'flex-start',maxWidth:'720px',margin:'0 auto'}},
          h('div',{className:'msg-ai-3d',style:{borderRadius:'18px 18px 18px 4px',padding:'12px 16px',display:'inline-flex',alignItems:'center',gap:'8px'}},
    ...
    h('div',{style:{flexShrink:0,borderTop:'1px solid '+BORDER,padding:'10px 12px 10px',background:BG}},
      h('div',{style:{maxWidth:'720px',margin:'0 auto'}},
        ...
        h('div',{className:'input-3d',style:{border:'1.5px solid '+(listening?RED:BORDER),borderRadius:'20px',padding:'13px 15px'}},
          h('textarea',{
