...
var MODELS=[
  ...
  {id:'openrouter',name:'OpenRouter',icon:'◈', color:'#8B6FE8',free:true,  builtin:true}
];
...
function callOpenRouterDirect(msgs) {
  ...
  return fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    body: JSON.stringify({ model: 'deepseek/deepseek-chat-v3-0324:free', max_tokens: 1200, messages: m })
  }).then(function(r) { return r.json(); }).then(function(d) {
    var t = '';
    try { t = d.choices[0].message.content; } catch(e) { t = JSON.stringify(d).slice(0,300); }
    return { reply: t || 'No response.', model: 'openrouter' };
  });
}
...
function callAI(modelId, msgs, keys, userText) {
  ...
  if (eff === 'openrouter') {
    return callOpenRouterDirect(msgs);
  }
  // Claude - needs worker
