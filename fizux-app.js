...
function callHF(msgs, model, hfToken) {
  ...
  var headers = {
    ...
    'Authorization': 'Bearer ' + (hfToken || 'hf_anonymous')
  };
  ...
  return fetch('https://router.huggingface.co/v1/chat/completions', {
    ...
    return fetch('https://api-inference.huggingface.co/models/mistralai/Mistral-7B-Instruct-v0.3', {
      method: 'POST',
      headers: {'Content-Type': 'application/json', 'Authorization': 'Bearer ' + (hfToken || 'hf_anonymous')},
      ...
      h('div',{style:{width:'275px',height:'100%',display:'flex',flexDirection:'column',overflow:'hidden'}},
        ...
        h('div',{style:{flex:1,overflowY:'auto',padding:'0 10px'}},
          ...
          FEATURES.map(function(f){
            return h('button',{key:f.id,onClick:function(){setInput(function(prev){var tag='['+f.label+']';return prev?prev+' '+tag:tag;});setSidebar(false);if(inputRef.current){inputRef.current.focus();inputRef.current.scrollTop=inputRef.current.scrollHeight;}},
