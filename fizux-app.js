...
function callAI(modelId, msgs, keys, userText) {
  // Read HF token from AFZUX (optional - for HF models)
  var hfToken = '';
  try {
  ...
  var polModel = pickModel(modelId, userText || '');
  return callPollinations(msgs, polModel, userText || '');
}

function pickModel(modelId, text) {
  var t = text.toLowerCase();
  // Pollinations supports: openai, mistral, llama, deepseek, qwen
...
function callPollinations(msgs, model, userText) {
  var messages = msgs.slice(-12).map(function(m) {
  ...
  return fetch('https://text.pollinations.ai/', {
    ...
    return fetch('https://text.pollinations.ai/', {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        messages: msgs.slice(-6).map(function(m){return{role:m.role,content:String(m.content||'')}}),
        model: 'openai',
        seed: 42
      })
    }).then(function(r2) { return r2.text(); }).then(function(t2) {
      return { reply: (t2||'').trim() || 'Please try again.', model: 'openai' };
    }).catch(function() {
      return { reply: 'Connection error. Please try again.', model: 'error' };
    });
  });
}
...
function FIZUXApp(){
  ...
  function genFollowUps(reply){
    fetch('https://text.pollinations.ai/',{
      ...
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({
        messages:[{role:'user',content:'Give exactly 3 short follow-up questions (max 8 words each) based on: "'+reply.slice(0,200)+'". Reply with ONLY a JSON array: ["q1","q2","q3"]'}],
