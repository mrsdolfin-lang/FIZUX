...
function callViaWorker(endpoint,msgs,apiKey){
  ...
  if(!workerUrl) return Promise.reject(new Error('Worker URL not set. Go to Settings → AI tab and add your Cloudflare Worker URL.'));
  ...
  return fetch(url,{
    ...
    if(d.content){
      var reply='',searched=false;
      (d.content||[]).forEach(function(b){ if(b.type==='text')reply+=b.text; if(b.type==='tool_use')searched=true; });
      return {reply:reply||'No response.',searched:searched,model:'claude'};
    }
    // Simple format
    return {reply:d.reply||'No response.',searched:false,model:d.model||endpoint};
  });
}
...
function callAI(modelId,msgs,keys,userText){
  ...
  if(modelId==='auto'){
    ...
    if(/sql|database/.test(t)&&k.deepseekKey) eff='deepseek';
    else if(/translate|hindi/.test(t)&&k.geminiKey) eff='gemini';
    else eff='claude';
  }
  if(eff==='gemini') return callViaWorker('gemini',msgs,k.geminiKey);
  if(eff==='deepseek') return callViaWorker('deepseek',msgs,k.deepseekKey);
  if(eff==='gpt') return callViaWorker('openai',msgs,k.openaiKey);
  if(eff==='grok') return callViaWorker('grok',msgs,k.grokKey);
  return callViaWorker('claude',msgs,k.claudeKey);
}
