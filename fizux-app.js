...
var MODELS=[
  ...
  {id:'mistral',  name:'Mistral',     icon:'🌬',color:'#8B6FE8',free:true},
  {id:'deepseek', name:'DeepSeek V3', icon:'⬡',color:'#2ECC8A',free:true},
  {id:'llama',    name:'Llama 3',     icon:'🦙',color:'#FF8C42',free:true},
  {id:'qwen',     name:'Qwen 2.5',    icon:'🔮',color:'#FF6B9D',free:true},
  {id:'r1',       name:'DeepSeek R1', icon:'🧠',color:'#2ECC8A',free:true}
];
...
// Worker URL stored in localStorage as 'fizux:workerUrl'
// Format: https://fizux-api.yourname.workers.dev

function getWorkerUrl(){
  try{ return localStorage.getItem('lc_fizux:workerUrl')||''; }catch(e){ return ''; }
}

// No Cloudflare Worker - using OpenRouter


function callAI(modelId, msgs, keys, userText) {
  // Read keys from AFZUX shared storage
  var k = {};
  try {
  ...
  var hfModels = ['mistral','deepseek','llama','qwen','r1','auto'];
  if (hfModels.indexOf(eff) > -1) {
    return callHuggingFace(msgs, eff, k.hfToken || '');
  }

  return Promise.reject(new Error('Model not available.'));
}

function callHuggingFace(msgs, modelKey, hfToken) {
  var HF_MODELS = {
  ...
  var model = HF_MODELS[modelKey] || HF_MODELS['mistral'];
  var prompt = '';
  ...
  return fetch('https://api-inference.huggingface.co/models/' + model, {
    ...
    var t = '';
    try { t = Array.isArray(d) ? d[0].generated_text : (d.generated_text || ''); } catch(e) {}
    if (!t && d.error) return { reply: 'Model loading, please try again in a moment.', model: modelKey };
    return { reply: (t || 'No response.').trim(), model: modelKey };
  }).catch(function(e) {
    return { reply: 'Error: ' + e.message, model: modelKey };
  });
}
...
function FIZUXApp(){
  ...
  function genFollowUps(reply){
    var hfToken='';
    try{var sh=localStorage.getItem('sh_fizux:aikeys');if(sh){var kk=JSON.parse(sh);hfToken=kk.hfToken||'';}}catch(e){}
    var hfHeaders={'Content-Type':'application/json'};
    ...
    fetch('https://api-inference.huggingface.co/models/mistralai/Mistral-7B-Instruct-v0.3',{
      ...
      body:JSON.stringify({inputs:'Give 3 short follow-up questions (max 8 words each) for: "'+reply.slice(0,200)+'". Reply with JSON array only: ["q1","q2","q3"]',parameters:{max_new_tokens:100,return_full_text:false}})
    }).then(function(r){return r.json();}).then(function(d){
      var text='';
      try{text=Array.isArray(d)?d[0].generated_text:(d.generated_text||'');}catch(e){}
      setFollowUps([]);
  ...
  function renderMsg(msg,i){
    ...
    var mi=!isUser?mInfo(msg.model||'mistral'):null;
