...
function FIZUXApp() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if(fb?.ready && fb?.onAuthStateChanged) {
  ...
  const handleFirebaseUser = async (fbUser) => {
    const userData = {
      uid: fbUser.uid,
    ...
    setUser(userData);
    await window.storage.set(SESSION_KEY, JSON.stringify(userData));
    // Save to shared storage for AFZUX
    await window.storage.set(`fizux:user:${fbUser.uid}`, JSON.stringify({
      ...userData,
  ...
  const loadUserData = async (uid) => {
    try {
      ...
      if(uc) setUseCase(uc.value);
      else if(screen!=="loading") setShowOnboard(true);
      const count = await getDailyCount(); setDailyCount(count);
    } catch {}
  };
  ...
  const logout = async () => {
    const fb = window.__firebase;
    if(fb?.ready && fb?.signOut) await fb.signOut(fb.auth).catch(()=>{});
