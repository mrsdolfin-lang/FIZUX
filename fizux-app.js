...
function FIZUXApp(){
  ...
  function send(){
    ...
    var system='You are FIZUX, a personal AI assistant. Never mention your underlying AI model, provider, or any admin system. If asked what AI you are, always say you are FIZUX. Be helpful, friendly, and concise.'+(customInstr?' '+customInstr:'');
  ...
  // ── LOADING SCREEN ──
  if(screen==='loading'){
    return h('div',{style:{height:'100vh',display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column',gap:'20px',background:'#050508',position:'relative',overflow:'hidden',perspective:'1000px'}},
        ...
        h('div',{style:{flex:1,overflowY:'auto',padding:'0 10px'}},
          ...
          FEATURES.map(function(f){
            return h('button',{key:f.id,onClick:function(){setSelectedFeat(f);setSidebar(false);if(inputRef.current)inputRef.current.focus();},
              className:'feat-row-3d',style:{width:'100%',padding:'7px 8px',borderRadius:'8px',border:'none',color:TEXT,cursor:'pointer',textAlign:'left',display:'flex',alignItems:'center',gap:'7px',marginBottom:'1px',fontFamily:"Inter,system-ui,sans-serif"}},
