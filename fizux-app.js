...
var OR_FREE = 'mistralai/mistral-7b-instruct:free';
...
function callOpenRouterDirect(msgs, modelKey) {
  ...
  return fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    body: JSON.stringify({ model: OR_FREE, max_tokens: 1200, messages: m })
  }).then(function(r) { return r.json(); }).then(function(d) {
    var t = '';
    try { t = d.choices[0].message.content; } catch(e) {}
    if (!t) throw new Error(JSON.stringify(d).slice(0,100));
...
function callClaudeDirect(msgs, system) {
  // Try direct Anthropic first, fallback to OR
  ...
  return fetch('https://api.anthropic.com/v1/messages', {
    ...
    headers: {
      'Content-Type': 'application/json',
      'x-api-key': 'sk-ant-api03-3vxrFVadYHRrcywQ3Px7kzmR8yjU8F_ibJ-pK4fU5VStIQOjXMpB2FzvHluIORijaZmMP70fSNxB47DLh6Uwdw-79ioVAAA',
      'anthropic-version': '2023-06-01',
      'anthropic-dangerous-direct-browser-access': 'true'
    },
    body: JSON.stringify({ model: 'claude-haiku-4-5', max_tokens: 1200, system: sys, messages: m })
  }).then(function(r) { return r.json(); }).then(function(d) {
    var t = '';
    ...
    return callOpenRouterDirect(msgs, 'mistral');
  });
}
...
function FIZUXApp(){
  ...
  function genFollowUps(reply){
    fetch('https://openrouter.ai/api/v1/chat/completions',{
      method:'POST',
      headers:{'Content-Type':'application/json','Authorization':'Bearer '+OR_KEY,'HTTP-Referer':'https://mrsdolfin-lang.github.io/FIZUX/'},
      body:JSON.stringify({model:OR_FREE,max_tokens:80,
