...
function getAIKeys() {
  var k = {groqKeys:[],orKeys:[],hfToken:'',geminiKey:'',claudeKey:'',openaiKey:'',grokKey:''};
  try {
    var s = localStorage.getItem('sh_fizux:aikeys');
    if (s) {
      var parsed = JSON.parse(s);
      k.groqKeys = (parsed.groqKeys||[]).filter(function(x){return x&&x.trim();});
      k.orKeys = (parsed.orKeys||[]).filter(function(x){return x&&x.trim();});
...
function getORKey() {
  var k = getAIKeys();
  if (!k.orKeys.length) return '';
  var idx = 0;
  try { idx = parseInt(localStorage.getItem('fizux:orIdx')||'0') % k.orKeys.length; } catch(e){}
  var key = k.orKeys[idx];
  try { localStorage.setItem('fizux:orIdx', String((idx+1)%k.orKeys.length)); } catch(e){}
  return key;
}
...
function callAI(modelId, msgs, keys, userText) {
  ...
  if (modelId === 'fast') {
    ...
    var orKey = getORKey();
    // FAST: only Groq or OR - no Pollinations fallback
    // Premium users: Groq priority always (fastest)
    // Free users: OR first, Groq as backup
    var useGroqFirst = isPremium || !orKey;
    if (useGroqFirst && groqKey) {
      return callGroq(msgs,taskModel,groqKey).catch(function(){
        if (orKey) return callOR(msgs,taskModel,orKey).catch(function(){
          return Promise.resolve({reply:'⚡ Fast servers busy. Try again or switch to SLOW.',model:'error'});
        });
        return Promise.resolve({reply:'⚡ Fast servers busy. Try again or switch to SLOW.',model:'error'});
      });
    }
    if (orKey) {
      return callOR(msgs,taskModel,orKey).catch(function(){
        if (groqKey) return callGroq(msgs,taskModel,groqKey).catch(function(){
          return Promise.resolve({reply:'⚡ Fast servers busy. Try again or switch to SLOW.',model:'error'});
        });
        return Promise.resolve({reply:'⚡ Fast servers busy. Try again or switch to SLOW.',model:'error'});
      });
    ...
    // No keys set - inform user
    return Promise.resolve({reply:'⚡ No fast API keys set. Go to AFZUX → AI Keys to add Groq or OpenRouter keys.',model:'error'});
  }
  // SLOW - always Pollinations (unlimited, free)
  return callPollinations(msgs,taskModel);
}
...
function callOR(msgs, model, apiKey) {
  var OR_MODELS = {
  ...
  var orModel = OR_MODELS[model] || 'mistralai/mistral-7b-instruct:free';
  var messages = msgs.slice(-12).map(function(m){return{role:m.role,content:String(m.content||'')};});
  return fetch('https://openrouter.ai/api/v1/chat/completions',{
    method:'POST',
    headers:{'Content-Type':'application/json','Authorization':'Bearer '+apiKey,'HTTP-Referer':'https://mrsdolfin-lang.github.io/FIZUX/','X-Title':'FIZUX'},
    body:JSON.stringify({model:orModel,messages:messages,max_tokens:1024,temperature:0.7})
  }).then(function(r){
    ...
    try{t=d.choices[0].message.content;}catch(e){}
    if(!t) throw new Error('empty');
    return {reply:t.trim(),model:'OR'};
  });
}

function callPollinations(msgs, model) {
  var messages = msgs.slice(-12).map(function(m){return{role:m.role,content:String(m.content||'')};});
  return fetch('https://text.pollinations.ai/',{
    method:'POST',
    headers:{'Content-Type':'application/json'},
    body:JSON.stringify({messages:messages,model:model||'openai',seed:Math.floor(Math.random()*1000),jsonMode:false,stream:false,max_tokens:1024,temperature:0.7})
  }).then(function(r){
    ...
    return {reply:reply,model:'AI'};
  }).catch(function(){
    // Silent retry once
    return new Promise(function(res){
      setTimeout(function(){
        fetch('https://text.pollinations.ai/',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({messages:messages,model:model||'openai',seed:Math.floor(Math.random()*9999),stream:false,max_tokens:1024})})
        .then(function(r){return r.text();})
        .then(function(t){res({reply:(t||'').trim()||'...',model:'AI'});})
        .catch(function(){res({reply:'Network error. Please check connection.',model:'error'});});
      },2000);
    });
  });
}
...
function FIZUXApp(){
  ...
  function send(){
    ...
    callAI(model,apiMsgs,apiKeys,userText).then(function(result){
      ...
      setMessages(newMsgs.concat([{role:'assistant',content:'Sorry, there was an error: '+(e.message||'Please try again later.'),ts:Date.now()}]));
    }).then(function(){
      setLoading(false);
    });
  }
  ...
  function saveChat(msgs,title){
    ...
    if (groqKey) {
      callGroq(msgs,'openai',groqKey).then(function(r){parseFollowUps(r.reply);}).catch(function(){
        callPollinations(msgs,'openai').then(function(r){parseFollowUps(r.reply);}).catch(function(){setFollowUps([]);});
      });
    } else if (orKey) {
      callOR(msgs,'mistral',orKey).then(function(r){parseFollowUps(r.reply);}).catch(function(){
        callPollinations(msgs,'openai').then(function(r){parseFollowUps(r.reply);}).catch(function(){setFollowUps([]);});
      });
    } else {
      callPollinations(msgs,'openai').then(function(r){parseFollowUps(r.reply);}).catch(function(){setFollowUps([]);});
  ...
  function btn(ex){return Object.assign({padding:'10px 16px',borderRadius:'10px',background:'#111111',color:'#FFFFFF',fontWeight:700,fontSize:'13px',cursor:'pointer',border:'none',fontFamily:'inherit'},ex||{});}
  ...
  if(screen==='loading'){
    return h('div',{style:{height:'100vh',display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column',gap:'16px',background:'#0A0A0F'}},
      h('div',{style:{width:'80px',height:'80px',borderRadius:'24px',background:'#111111',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'36px',fontWeight:800,color:'#FFFFFF',boxShadow:'0 4px 20px rgba(201,168,76,0.3)'}},'F'),
      ...
      settingsTab==='account' && h('div',null,
        ...
        isPremium ? h('div',{style:card({marginBottom:'12px',borderColor:'rgba(0,0,0,0.4)',background:'rgba(0,0,0,0.08)'})},
          ...
          model==='fast' && h('div',{style:{height:'5px',background:BORDER,borderRadius:'3px',overflow:'hidden'}},
            h('div',{style:{height:'100%',width:Math.min(100,(fastDaily/(isPremium?FAST_DAILY_PREMIUM:FAST_DAILY))*100)+'%',background:fastDaily>=FAST_DAILY?RED:GREEN,borderRadius:'3px'}})
          ),
  ...
  function renderMsg(msg,i){
    ...
    return h('div',{key:i,style:{display:'flex',justifyContent:isUser?'flex-end':'flex-start',padding:'4px 16px',maxWidth:'720px',margin:'0 auto'}},
      h('div',{style:{
        maxWidth:'75%',
        background:isUser?'#000000':'#1A1A2E',
        color:'#FFFFFF',
        ...
        !isUser && h('div',{style:{display:'flex',gap:'8px',marginTop:'6px'}},
          h('button',{onClick:function(){try{navigator.clipboard.writeText(msg.content);}catch(e){}notify('Copied!');},
            style:{background:'none',border:'none',color:'#555',cursor:'pointer',fontSize:'11px',padding:'0',fontFamily:'inherit'}},'Copy'),
          'speechSynthesis' in window && h('button',{onClick:function(){var u=new SpeechSynthesisUtterance(msg.content);window.speechSynthesis.speak(u);},
            style:{background:'none',border:'none',color:'#555',cursor:'pointer',fontSize:'11px',padding:'0',fontFamily:'inherit'}},'Read')
        )
      )
    );
  }
  ...
  return h('div',{
        ...
        ['slow','fast'].map(function(spd){
          ...
          return h('button',{key:spd,onClick:function(){setModel(spd);},
            style:{padding:'5px 14px',borderRadius:'8px',border:'none',
              ...
              fontWeight:act?700:400,fontSize:'12px',cursor:'pointer',fontFamily:'inherit',
              transition:'all 0.2s'}},
            spd==='slow'?'🐢 SLOW':'⚡ FAST'
          );
        })
    ...
    h('div',{style:{flex:1,overflowY:'auto',padding:'0'}},
      h('div',{style:{maxWidth:'720px',margin:'0 auto'}},
        messages.length===0
          ? h('div',{style:{padding:'40px 16px',textAlign:'center'}},
              h('div',{style:{fontSize:'32px',marginBottom:'12px'}},'✦'),
              h('div',{style:{fontSize:'16px',fontWeight:700,color:TEXT,marginBottom:'6px'}},'How can I help you?'),
              h('div',{style:{fontSize:'12px',color:MUTED}},'Select a feature or type a message below')
            )
        ...
        followUps.length>0&&!loading && h('div',{style:{padding:'8px 16px 0',display:'flex',gap:'6px',flexWrap:'wrap'}},
          followUps.map(function(q,i){
            return h('button',{key:i,onClick:function(){setInput(q);setTimeout(function(){send();},50);},
              style:{background:CARD,border:'1px solid '+BORDER,borderRadius:'20px',padding:'5px 12px',fontSize:'11px',color:SUB,cursor:'pointer',fontFamily:'inherit'}},q);
          })
        ),
