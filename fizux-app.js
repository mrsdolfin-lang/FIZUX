...
function callGemini(msgs, model, apiKey) {
  ...
  return fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-lite:generateContent?key='+apiKey,{
    ...
    return {reply:t.trim(),model:'Gemini'};
  });
}
  ...
  function tryGeminiFallback() {
    if (geminiKey && geminiKey.length > 10) {
      return callGemini(msgs, taskModel, geminiKey).catch(function(){
        return Promise.resolve({reply:'⚠️ All AI services busy. Please try again in a moment.',model:'error'});
      });
    }
    return Promise.resolve({reply:'⚠️ AI service unavailable. Add keys in AFZUX → AI Keys.',model:'error'});
  }
...
function FIZUXApp(){
  ...
  // ── LOGIN SCREEN ──
  if(screen==='login'){
    return h('div',{style:{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',padding:'20px',background:'#050508',position:'relative',overflow:'hidden',perspective:'1000px'},className:'screen-fade'},
      ...
      h('div',{style:{width:'100%',maxWidth:'400px'}},
        ...
        // Card
        h('div',{className:'login-card-3d',style:{borderRadius:'24px',padding:'32px'}},
          // Google btn
          h('button',{
