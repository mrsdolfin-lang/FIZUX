...
function callAI(modelId, msgs, keys, userText) {
  // Read HF token from AFZUX shared storage
  var hfToken = '';
  try {
    var s = localStorage.getItem('sh_fizux:aikeys');
    if (s) { var k = JSON.parse(s); hfToken = k.hfToken || ''; }
  } catch(e) {}

  // Pick model based on selection + feature detection
  var model = pickModel(modelId, userText || '');
  return callHF(msgs, model, hfToken);
}

function pickModel(modelId, text) {
  var t = text.toLowerCase();

  // If specific model selected by user
  var modelMap = {
  ...
  if (modelId !== 'auto' && modelMap[modelId]) {
    return modelMap[modelId];
  }

  // Auto mode: pick best model based on task
  if (/code|sql|program|function|debug|html|css|javascript|python|java|php/.test(t)) {
    return 'Qwen/Qwen2.5-Coder-7B-Instruct';
  }
  if (/math|calculat|equation|solve|algebra|geometry|statistic|formula|proof/.test(t)) {
    return 'Qwen/Qwen2.5-Math-7B-Instruct';
  }
  if (/translat|hindi|urdu|french|spanish|arabic|chinese|japanese|german/.test(t)) {
    return 'mistralai/Mistral-7B-Instruct-v0.3';
  }
  if (/reason|analyze|deep|think|compare|explain why|pros cons/.test(t)) {
...
function callHF(msgs, model, hfToken) {
  // Use HuggingFace router - OpenAI compatible API
  var messages = msgs.slice(-10).map(function(m) {
  ...
  return fetch('https://router.huggingface.co/v1/chat/completions', {
    ...
    return { reply: (t || 'Please try again.').trim(), model: model.split('/').pop() };
  }).catch(function(e) {
    // Fallback to api-inference endpoint
    var prompt = '';
    msgs.slice(-6).forEach(function(m) {
      prompt += (m.role === 'user' ? 'Human: ' : 'Assistant: ') + m.content + '\n';
    });
    prompt += 'Assistant:';
    return fetch('https://api-inference.huggingface.co/models/mistralai/Mistral-7B-Instruct-v0.3', {
      method: 'POST',
      headers: hfToken ? {'Content-Type': 'application/json', 'Authorization': 'Bearer ' + hfToken} : {'Content-Type': 'application/json'},
      body: JSON.stringify({inputs: prompt, parameters: {max_new_tokens: 512, return_full_text: false}})
    }).then(function(r2) { return r2.json(); }).then(function(d2) {
      var t2 = '';
      try { t2 = Array.isArray(d2) ? d2[0].generated_text : (d2.generated_text || ''); } catch(e) {}
      return { reply: (t2 || 'Please try again.').trim(), model: 'Mistral' };
    }).catch(function() {
      return { reply: 'Please check your HuggingFace token in AFZUX → AI Keys.', model: 'error' };
    });
  });
}
  ...
  var SettingsModal=showSettings ? h('div',{style:overlay()},
    h('div',{style:modalStyle({maxWidth:'440px'})},
      ...
      settingsTab==='ai' && h('div',null,
        h('div',{style:{background:'rgba(46,204,138,0.08)',border:'1px solid rgba(46,204,138,0.2)',borderRadius:'14px',padding:'20px',textAlign:'center'}},
          h('div',{style:{fontSize:'28px',marginBottom:'10px'}},'🤖'),
          h('div',{style:{fontSize:'14px',fontWeight:700,color:GREEN,marginBottom:'6px'}},'AI Models Ready'),
          h('div',{style:{fontSize:'12px',color:MUTED,lineHeight:1.6}},'All AI models are configured by admin.')
        )
      ),
      // APPEARANCE
  ...
  return h('div',{
    ...
    h('div',{style:{flex:1,overflowY:'auto',padding:'0'}},
      h('div',{style:{maxWidth:'720px',margin:'0 auto'}},
        ...
        loading && h('div',{style:{padding:'16px 16px',display:'flex',gap:'12px',alignItems:'flex-start'}},
          h('div',{style:{width:'34px',height:'34px',borderRadius:'50%',background:curModel.color+'33',border:'1px solid '+curModel.color+'66',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'14px',flexShrink:0}},curModel.icon),
          h('div',{style:{display:'flex',gap:'5px',alignItems:'center',padding:'8px 0'}},
