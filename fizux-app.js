...
function callAI(modelId, msgs, keys, userText) {
  ...
  if (modelId === 'fast') {
    ...
    var orKey = getORKey();
    // Read premium status from localStorage (set when activated)
    var premStatus = false;
    try { premStatus = localStorage.getItem('fizux:ispremium')==='1'; } catch(e){}
    // Premium: Groq first (fastest). Free: OR first (save Groq quota)
    var useGroqFirst = premStatus || !orKey;
    if (useGroqFirst && groqKey && groqKey.length > 10) {
...
function FIZUXApp(){
  ...
  function send(){
    ...
    callAI(model,apiMsgs,apiKeys,userText).then(function(result){
      ...
      if(model!=='fast') bumpDaily().then(function(c){setDaily(c);}).catch(function(){});
