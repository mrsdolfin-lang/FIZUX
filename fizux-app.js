...
function callAI(modelId, msgs, keys, userText) {
  ...
  if (modelId === 'fast') {
    ...
    if (groqKey) return callGroq(msgs,taskModel,groqKey).catch(function(){
      if(orKey) return callOR(msgs,taskModel,orKey).catch(function(){return callPollinations(msgs,taskModel);});
...
function callPollinations(msgs, model) {
  ...
  return fetch('https://text.pollinations.ai/',{
    ...
    return {reply:'Please try again.',model:'error'};
  });
}
...
function FIZUXApp(){
  ...
  function canSend(){ if(isPremium) return true; if(model==='fast') return fastDaily<FAST_DAILY; return daily<FREE_DAILY; }
  function remaining(){ if(isPremium) return '∞'; if(model==='fast') return Math.max(0,FAST_DAILY-fastDaily)+' fast'; return Math.max(0,FREE_DAILY-daily)+' free'; }
