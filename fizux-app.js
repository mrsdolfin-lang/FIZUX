...
function callClaude(msgs){
  return fetch('https://api.anthropic.com/v1/messages',{
    method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({model:'claude-sonnet-4-20250514',max_tokens:1200,
      system:'You are FIZUX, a helpful AI assistant. Be precise, helpful, and thorough.',
...
function callGemini(msgs,key){
  if(!key) return Promise.reject(new Error('No Gemini key'));
  var contents=msgs.slice(-20).map(function(m){
    return {role:m.role==='user'?'user':'model',parts:[{text:typeof m.content==='string'?m.content:'[content]'}]};
  });
  return fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key='+key,{
    ...
    return {reply:t||'No response.',searched:false,model:'gemini'};
  });
}

function callDeepSeek(msgs,key){
  if(!key) return Promise.reject(new Error('No DeepSeek key'));
  return fetch('https://api.deepseek.com/v1/chat/completions',{
    method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+key},
    body:JSON.stringify({model:'deepseek-chat',max_tokens:1200,
      messages:msgs.slice(-20).map(function(m){return {role:m.role,content:typeof m.content==='string'?m.content:'[content]'};})})
  }).then(function(r){return r.json();}).then(function(d){
    var t=d.choices&&d.choices[0]&&d.choices[0].message&&d.choices[0].message.content;
    return {reply:t||'No response.',searched:false,model:'deepseek'};
  });
}

function callOpenAI(msgs,key){
  if(!key) return Promise.reject(new Error('No OpenAI key'));
  return fetch('https://api.openai.com/v1/chat/completions',{
    ...
    var t=d.choices&&d.choices[0]&&d.choices[0].message&&d.choices[0].message.content;
    return {reply:t||'No response.',searched:false,model:'gpt'};
  });
}

function callGrok(msgs,key){
  if(!key) return Promise.reject(new Error('No Grok key'));
  return fetch('https://api.x.ai/v1/chat/completions',{
    ...
    var t=d.choices&&d.choices[0]&&d.choices[0].message&&d.choices[0].message.content;
    return {reply:t||'No response.',searched:false,model:'grok'};
  });
}
...
function callAI(modelId,msgs,keys,userText){
  ...
  if(eff==='gemini'&&k.geminiKey) return callGemini(msgs,k.geminiKey);
  if(eff==='deepseek'&&k.deepseekKey) return callDeepSeek(msgs,k.deepseekKey);
  if(eff==='gpt'&&k.openaiKey) return callOpenAI(msgs,k.openaiKey);
  if(eff==='grok'&&k.grokKey) return callGrok(msgs,k.grokKey);
  return callClaude(msgs);
}
