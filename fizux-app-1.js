...
var MODELS=[
  {id:'auto',   name:'Auto',       icon:'⚡',color:'#C9A84C',free:true},
  {id:'claude', name:'Claude',     icon:'◆', color:'#C9A84C',free:true},
  {id:'gemini', name:'Gemini',     icon:'✦', color:'#4A9EFF',free:true,  key:'geminiKey'},
  {id:'deepseek',name:'DeepSeek',  icon:'⬡', color:'#2ECC8A',free:true,  key:'deepseekKey'},
...
function callViaWorker(endpoint, msgs, apiKey) {
  var workerUrl = '';
  try { workerUrl = localStorage.getItem('lc_fizux:workerUrl') || ''; } catch(e) {}
  if (!workerUrl) return Promise.reject(new Error('No worker URL'));
  var url = workerUrl.replace(/\/$/, '') + '/' + endpoint;
  return fetch(url, {
    method: 'POST',
    ...
    if (d.content) {
      var reply = '';
      (d.content || []).forEach(function(b) { if (b.type === 'text') reply += b.text; });
      return { reply: reply || 'No response.', model: 'claude' };
    }
    return { reply: d.reply || 'No response.', model: d.model || endpoint };
  });
}
...
function callAI(modelId, msgs, keys, userText) {
  ...
  if (modelId === 'auto') {
    ...
    else if (k.deepseekKey) eff = 'deepseek';
    else if (k.openaiKey) eff = 'gpt';
    else eff = 'gemini';
  }

  // Try worker first if URL set, else direct
  var workerUrl = '';
  try { workerUrl = localStorage.getItem('lc_fizux:workerUrl') || ''; } catch(e) {}

  if (eff === 'gemini') {
    if (workerUrl) return callViaWorker('gemini', msgs, k.geminiKey || '').catch(function() { return callGeminiDirect(msgs, k.geminiKey); });
    return callGeminiDirect(msgs, k.geminiKey);
  }
  if (eff === 'deepseek') {
    if (workerUrl) return callViaWorker('deepseek', msgs, k.deepseekKey || '').catch(function() { return callDeepSeekDirect(msgs, k.deepseekKey); });
    return callDeepSeekDirect(msgs, k.deepseekKey);
  }
  if (eff === 'gpt') {
    if (workerUrl) return callViaWorker('openai', msgs, k.openaiKey || '').catch(function() { return callOpenAIDirect(msgs, k.openaiKey); });
    return callOpenAIDirect(msgs, k.openaiKey);
  }
  if (eff === 'grok') {
    if (workerUrl) return callViaWorker('grok', msgs, k.grokKey || '').catch(function() { return callGrokDirect(msgs, k.grokKey); });
    return callGrokDirect(msgs, k.grokKey);
  }
  // Claude - needs worker
  if (workerUrl) return callViaWorker('claude', msgs, k.claudeKey || '');
  return Promise.reject(new Error('Claude needs Cloudflare Worker. Use Gemini instead — add Gemini key in Settings → AI tab.'));
}
...
function FIZUXApp(){
  ...
  var s36=useState(''); var workerUrl=s36[0]; var setWorkerUrl=s36[1];
  ...
  useEffect(function(){
    ...
    ST.get('fizux:apikeys').then(function(d){try{setApiKeys(JSON.parse(d.value));}catch(e){}}).catch(function(){});
    ST.get('fizux:workerUrl').then(function(d){setWorkerUrl(d.value);}).catch(function(){});
  ...
  // ── WORKER URL BANNER (shown in chat if not set) ──
  var showWorkerBanner = screen==='chat' && !workerUrl;
  ...
  var SettingsModal=showSettings ? h('div',{style:overlay()},
    h('div',{style:modalStyle({maxWidth:'440px'})},
      ...
      settingsTab==='ai' && h('div',null,
        h('div',{style:{fontSize:'11px',color:MUTED,marginBottom:'12px'}},'Add API keys to unlock more AI models. Stored locally on your device.'),
          h('div',{style:{marginBottom:'14px',background:GOLD+'11',border:'1px solid '+GOLD+'44',borderRadius:'12px',padding:'12px'}},
            h('div',{style:{fontSize:'11px',color:GOLD,fontWeight:700,marginBottom:'6px'}},'⚡ CLOUDFLARE WORKER URL'),
            h('div',{style:{fontSize:'10px',color:MUTED,marginBottom:'8px'}},'Deploy fizux-worker.js to Cloudflare → paste URL here → all AI works'),
            h('div',{style:{display:'flex',gap:'6px'}},
              h('input',{value:workerUrl,onChange:function(e){setWorkerUrl(e.target.value.trim());},
                placeholder:'https://fizux-api.yourname.workers.dev',
                style:inpStyle({flex:1,fontSize:'11px',padding:'8px 10px'})}),
              h('button',{onClick:function(){ ST.set('fizux:workerUrl',workerUrl).catch(function(){}); notify('Worker URL saved!'); },style:btn({padding:'8px 12px',fontSize:'11px'})},'Save')
            ),
            workerUrl && h('div',{style:{fontSize:'10px',color:GREEN,marginTop:'5px'}},'✅ '+workerUrl.slice(0,45))
          ),
        [{id:'geminiKey',label:'Gemini API Key',link:'https://aistudio.google.com',free:true},
         {id:'deepseekKey',label:'DeepSeek Key',link:'https://platform.deepseek.com',free:true},
         ...
         {id:'grokKey',label:'Grok (xAI) Key',link:'https://console.x.ai',free:false}
          ...
          return h('div',{key:k.id,style:{marginBottom:'10px'}},
            ...
            h('div',{style:{display:'flex',gap:'6px'}},
              ...
              h('button',{onClick:function(){ST.set('fizux:apikeys',JSON.stringify(apiKeys)).catch(function(){});notify('Saved!');},style:btn({padding:'8px 12px',fontSize:'11px'})},'Save')
            )
          );
        })
      ),
  ...
  return h('div',{
    ...
    // Worker URL banner
    showWorkerBanner && h('div',{style:{background:'rgba(201,168,76,0.15)',borderBottom:'2px solid '+GOLD,padding:'10px 14px',flexShrink:0,display:'flex',gap:'10px',alignItems:'center',flexWrap:'wrap'}},
      h('span',{style:{fontSize:'12px',color:GOLD,fontWeight:700,flex:1}},'⚡ Add Cloudflare Worker URL to enable AI responses'),
      h('input',{
        value:workerUrl,
        onChange:function(e){setWorkerUrl(e.target.value.trim());},
        onKeyDown:function(e){if(e.key==='Enter'&&workerUrl){ST.set('fizux:workerUrl',workerUrl).catch(function(){});notify('Worker URL saved! Now send a message.');}},
        placeholder:'https://fizux-api.yourname.workers.dev',
        style:{background:CARD,border:'1.5px solid '+GOLD,borderRadius:'8px',padding:'7px 10px',color:TEXT,fontSize:'12px',fontFamily:'inherit',outline:'none',minWidth:'200px',flex:2}
      }),
      h('button',{
        onClick:function(){
          if(workerUrl){
            ST.set('fizux:workerUrl',workerUrl).catch(function(){});
            notify('✅ Worker URL saved!');
          }
        },
        style:btn({padding:'7px 14px',fontSize:'12px',borderRadius:'8px'})
      },'Save')
    ),
