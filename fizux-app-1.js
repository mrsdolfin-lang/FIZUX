...
function callGeminiDirect(msgs, key) {
  if (!key) return Promise.reject(new Error('Gemini API key needed. Contact admin to configure.'));
...
function callDeepSeekDirect(msgs, key) {
  if (!key) return Promise.reject(new Error('DeepSeek API key needed. Contact admin to configure.'));
...
function callOpenAIDirect(msgs, key) {
  if (!key) return Promise.reject(new Error('OpenAI API key needed. Contact admin to configure.'));
  var m = msgs.slice(-20).map(function(m) { return { role: m.role, content: String(m.content || '') }; });
...
function callGrokDirect(msgs, key) {
  if (!key) return Promise.reject(new Error('Grok API key needed. Contact admin to configure.'));
  var m = msgs.slice(-20).map(function(m) { return { role: m.role, content: String(m.content || '') }; });
...
// OR key loaded from user settings
var OR_KEY = '';
// OpenRouter free models
var OR_MODELS = {
  'deepseek':  'deepseek/deepseek-chat:free',
  'llama':     'meta-llama/llama-3.2-3b-instruct:free',
  'gemini-or': 'google/gemma-3-1b-it:free',
  'mistral':   'mistralai/mistral-7b-instruct:free',
  'qwen':      'qwen/qwen-2-7b-instruct:free',
  'r1':        'deepseek/deepseek-r1-distill-qwen-1.5b:free',
  'phi':       'microsoft/phi-3-mini-128k-instruct:free'
};

function callOpenRouterDirect(msgs, modelKey) {
  var orModel = OR_MODELS[modelKey] || 'mistralai/mistral-7b-instruct:free';
  // Read OR key fresh from storage
  var orKey = '';
  try { var s = localStorage.getItem('sh_fizux:aikeys'); if(s) { var k = JSON.parse(s); orKey = k.orKey||''; } } catch(e) {}
  if (!orKey) return Promise.reject(new Error('OpenRouter key needed. Contact admin to configure.'));
  var m = msgs.slice(-20).map(function(x) {
    return { role: x.role, content: String(x.content || '') };
  });
  return fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    if (!t) {
      ...
      return fetch('https://openrouter.ai/api/v1/chat/completions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + orKey, 'HTTP-Referer': 'https://mrsdolfin-lang.github.io/FIZUX/' },
        body: JSON.stringify({ model: 'mistralai/mistral-7b-instruct:free', max_tokens: 1200, messages: m })
      }).then(function(r2) { return r2.json(); }).then(function(d2) {
        var t2 = '';
        try { t2 = d2.choices[0].message.content; } catch(e) {}
        return { reply: t2 || 'No response. Try another model.', model: modelKey || 'ai' };
      });
    }
    return { reply: t, model: modelKey || 'ai' };
  });
}

// No Cloudflare Worker needed - all AI runs directly
// Claude via OpenRouter free
function callClaudeDirect(msgs, system) {
  var k = {};
  try { var s = localStorage.getItem('sh_fizux:aikeys'); if(s) k = JSON.parse(s)||{}; } catch(e) {}
  if (!k.claudeKey) return callOpenRouterDirect(msgs, 'mistral');
  var sys = system || 'You are FIZUX, a helpful AI assistant.';
  var m = msgs.slice(-20).map(function(x) { return { role: x.role, content: String(x.content || '') }; });
  return fetch('https://api.anthropic.com/v1/messages', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'x-api-key': k.claudeKey,
      'anthropic-version': '2023-06-01',
      'anthropic-dangerous-direct-browser-access': 'true'
    },
    body: JSON.stringify({ model: 'claude-sonnet-4-5', max_tokens: 1200, system: sys, messages: m })
  }).then(function(r) { return r.json(); }).then(function(d) {
    var t = '';
    try { for(var i=0;i<d.content.length;i++) { if(d.content[i].type==='text') t+=d.content[i].text; } } catch(e) {}
    if (!t) throw new Error('no reply');
    return { reply: t, model: 'claude' };
  }).catch(function() { return callOpenRouterDirect(msgs, 'mistral'); });
}

function callAI(modelId, msgs, keys, userText) {
  var k = keys || {};
  try {
    var stored = localStorage.getItem('sh_fizux:aikeys');
  ...
  if (modelId === 'auto') {
    if (k.geminiKey) eff = 'gemini';
    else if (k.openaiKey) eff = 'gpt';
    else if (k.orKey) eff = 'mistral';
    else eff = 'mistral';
  }

  // Models via OpenRouter (needs OR key in AFZUX)
  var freeModels = ['deepseek','llama','gemini-or','mistral','qwen','r1','phi'];
  if (freeModels.indexOf(eff) > -1) {
    if (!k.orKey) return Promise.reject(new Error('OpenRouter key not set. Contact admin.'));
  ...
  // Default fallback
  return Promise.reject(new Error('No AI model available. Contact admin.'));
}
...
function FIZUXApp(){
  ...
  useEffect(function(){
    ...
    // Load AI keys from AFZUX shared storage
    try{
      var shKeys=localStorage.getItem('sh_fizux:aikeys');
      if(shKeys){var k=JSON.parse(shKeys);if(k&&typeof k==='object')setApiKeys(k);}
    }catch(e){}
  ...
  function sendText(txt){
    ...
    var userText=txt.trim();
    var parts=[{type:'text',content:userText}];
    var userMsg={role:'user',content:userText,ts:Date.now()};
    setMessages(function(prev){return prev.concat([userMsg]);});
    setInput(''); setAttach([]); setFollowUps([]); setDetFeat(null);
    setLoading(true); setScreen('chat');
    var apiMsgs=[{role:'user',content:userText}];
    callAI(model,apiMsgs,apiKeys,userText).then(function(result){
      ...
      if(result.reply) saveChat(userMsg,aiMsg);
    }).catch(function(e){
      setMessages(function(prev){return prev.concat([{role:'assistant',content:'Error: '+(e.message||'Try again.'),ts:Date.now()}]);});
      setLoading(false);
    });
  }
  function send(){
  ...
  function genFollowUps(reply){
    fetch('https://openrouter.ai/api/v1/chat/completions',{
      method:'POST',
      headers:(function(){var ok='';try{var s=localStorage.getItem('sh_fizux:aikeys');if(s){var k=JSON.parse(s);ok=k.orKey||'';}}catch(e){}return{'Content-Type':'application/json','Authorization':'Bearer '+ok,'HTTP-Referer':'https://mrsdolfin-lang.github.io/FIZUX/'};})(),
      body:JSON.stringify({model:'mistralai/mistral-7b-instruct:free',max_tokens:80,
        messages:[{role:'user',content:'Give 3 short follow-up questions (max 8 words each) for: "'+reply.slice(0,200)+'". JSON array only: ["q1","q2","q3"]'}]})
    }).then(function(r){return r.json();}).then(function(d){
      var text='';
      try{text=d.choices[0].message.content;}catch(e){}
      try{var qs=JSON.parse(text.trim());setFollowUps(Array.isArray(qs)?qs.slice(0,3):[]);}catch(e){setFollowUps([]);}
  ...
  var SettingsModal=showSettings ? h('div',{style:overlay()},
    h('div',{style:modalStyle({maxWidth:'440px'})},
      ...
      settingsTab==='ai' && h('div',null,
        h('div',{style:{background:'rgba(46,204,138,0.08)',border:'1px solid rgba(46,204,138,0.2)',borderRadius:'12px',padding:'16px',textAlign:'center'}},
          h('div',{style:{fontSize:'24px',marginBottom:'8px'}},'🤖'),
          h('div',{style:{fontSize:'13px',fontWeight:700,color:GREEN,marginBottom:'6px'}},'AI Models Ready'),
          h('div',{style:{fontSize:'11px',color:MUTED,lineHeight:1.6}},'All AI models are configured by admin. No setup needed.')
        ),
      ...
      h('div',{style:{maxWidth:'720px',margin:'0 auto',padding:'0 16px'}},
        h('div',{style:{display:'flex',gap:'12px',alignItems:'flex-start'}},
          isUser
            ...
            : h('img',{src:'data:image/png;base64,',style:{width:'34px',height:'34px',borderRadius:'50%',objectFit:'cover',flexShrink:0}}),
      ...
      h('div',{style:{width:'275px',height:'100%',display:'flex',flexDirection:'column',overflow:'hidden'}},
        ...
        h('div',{style:{flex:1,overflowY:'auto',padding:'0 10px'}},
          ...
          FEATURES.map(function(f){
            return h('button',{key:f.id,onClick:function(){var fl=f.label;setSidebar(false);setTimeout(function(){sendText(fl);},100);},
        ...
        ? h('div',{style:{maxWidth:'720px',margin:'0 auto',padding:'24px 16px'}},
            ...
            h('div',{style:{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(100px,1fr))',gap:'7px',marginBottom:'20px'}},
              FEATURES.slice(0,12).map(function(f){
                return h('button',{key:f.id,onClick:function(){sendText(f.label);},
            ...
            h('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}},
              [['✉','Write a professional email','Office task'],
               ...
               ['🌟','Set my goals','Personal']
                ...
                return h('button',{key:item[0],onClick:function(){sendText(item[1]);},
            ...
            loading && h('div',{style:{padding:'16px 0'}},
              h('div',{style:{maxWidth:'720px',margin:'0 auto',padding:'0 16px'}},
                h('div',{style:{display:'flex',gap:'12px'}},
                  h('img',{src:'data:image/png;base64,',style:{width:'34px',height:'34px',borderRadius:'50%',objectFit:'cover'}}),
...
try {
  ...
  container.innerHTML = '<div style="display:flex;height:100vh;align-items:center;justify-content:center;flex-direction:column;gap:16px;background:#0A0A0F;color:#E8E8F8;font-family:sans-serif;text-align:center;padding:24px"><div style="font-size:48px">⚠</div><div style="font-size:20px;font-weight:700">FIZUX</div><div style="color:#888;font-size:13px">Refresh the page to try again</div><button onclick="location.reload()" style="margin-top:8px;padding:10px 24px;background:#C9A84C;color:#000;border:none;border-radius:8px;font-size:14px;font-weight:700;cursor:pointer">Refresh</button></div>';
}
})();
